...
import {ConstructingBeauty} from "../src/ConstructingBeauty.sol";

contract ConstructingBeautyTest is Test {
    ConstructingBeauty public cb;
    ...
    function testSetBaseURI() public {
        ...
        vm.prank(testingAddress);
        cb.setBaseURI(newBaseURI);

        // Verify that the base URI was updated correctly
        assertEq(cb.baseURI(), newBaseURI, "Base URI should match");
    }
    ...
    function testSetContractURI() public {
        ...
        vm.prank(testingAddress);
        cb.setContractURI(newContractURI);

        // Verify that the contract URI was updated correctly
        assertEq(cb.contractURI(), newContractURI, "Contract URI should match");
    }
    ...
    function testAirdropOneRecipient() public {
        ...
        vm.prank(testingAddress);
        cb.airdrop(recipients); 

        // Verify that the tokens were minted correctly
        assertEq(cb.totalSupply(), 1, "Total supply should be 1");
        assertEq(cb.ownerOf(0), recipients[0], "Token 0 should be owned by recipient 0");
    }
    ...
    function testAirdropMultipleRecipients() public {
        ...
        // Verify that the tokens were minted correctly
        assertEq(cb.totalSupply(), 3, "Total supply should be 3");
        assertEq(cb.ownerOf(0), recipients[0], "Token 0 should be owned by recipient 0");
        assertEq(cb.ownerOf(1), recipients[1], "Token 1 should be owned by recipient 1");
        assertEq(cb.ownerOf(2), recipients[2], "Token 2 should be owned by recipient 2");
    }
    ...
    function testAirdropGetTokenURI() public {
        ...
        assertEq(cb.tokenURI(0), "http://example.com/0.json", "Token URI should match");
    }
    ...
    function testRedeem() public {
        ...
        vm.prank(testingAddress);
        cb.redeem(0, redemptionId);

        // // Verify that the redemption was recorded correctly
        (string memory redeemedId, address redeemer) = cb.redeemed(0);
    ...
    function testRedeemReturnOwnershipOfToken() public {
        ...
        assertEq(cb.ownerOf(0), testingAddress, "Token 0 should be owned by testing address");
    }
    ...
    function testRedeemReturnRedeemingAddress() public {
        ...
        (, address redeemer) = cb.redeemed(0);
    ...
    function testRedeemReturnRedemptionId() public {
        ...
        (string memory redeemedId,) = cb.redeemed(0);
